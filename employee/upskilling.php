...
// Check if user has passed all general courses to unlock upskilling
// First, get all general courses
$sql_general = "SELECT c.id, c.title
                FROM courses c
...
$general_courses = $general_result->fetch_all(MYSQLI_ASSOC);
$stmt->close();

$total_general = count($general_courses);
$passed_general = 0;

foreach ($general_courses as $course) {
    // Get quiz stats for this general course
    ...
    // Get passing threshold for this course
    ...
    $cert_check_stmt->bind_param("ii", $user_id, $course['id']);
    $cert_check_stmt->execute();
    $has_certificate = $cert_check_stmt->get_result()->num_rows > 0;
    $cert_check_stmt->close();
    
    if ($is_passed || $has_certificate) {
        $passed_general++;
    }
}

$upskilling_unlocked = ($total_general > 0 && $passed_general == $total_general);

// Get all upskilling courses with user progress
...
$conditions = [];
if ($hasType) $conditions[] = "c.type = 'upskilling'";
...
// Fetch quiz results and retake status for each course (SAME LOGIC AS GENERAL COURSES)
$course_results = [];
foreach ($courses as $course) {
    ...
    $pending_stmt = $conn->prepare("SELECT id, status FROM retake_requests WHERE user_id = ? AND course_id = ? AND status = 'pending'");
    $pending_stmt->bind_param("ii", $user_id, $course['id']);
    $pending_stmt->execute();
    $has_pending_retake = $pending_stmt->get_result()->fetch_assoc();
    $pending_stmt->close();
    ...
    // If there's an approved retake, treat as not_started (SAME AS GENERAL COURSES)
    if ($has_approved_retake) {
        $course_status = 'not_started';
        $is_passed = false;
        $is_failed = false;
        $display_status = 'approved_retake';
    } elseif ($has_pending_retake) {
        $display_status = 'pending_retake';
        $is_passed = false;
        $is_failed = false;
    } else {
        // SAME LOGIC AS GENERAL COURSES
        ...
        $display_status = $is_passed ? 'passed' : ($is_failed ? 'failed' : $course_status);
    }
    ...
    $course_results[$course['id']] = [
        ...
        'has_pending_retake' => !empty($has_pending_retake),
...
<head>
    ...
    <title>Upskilling Courses - Upstaff Academy</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800&display=swap" rel="stylesheet">
    <style>
        * { font-family: 'Inter', sans-serif; }
        .course-card { transition: all 0.3s ease; height: 100%; display: flex; flex-direction: column; position: relative; }
        ...
        <div class="p-6" style="padding-top: 110px;">
            ...
            <?php if (!$upskilling_unlocked): ?>
                ...
                <div class="mb-6 bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3">
                    ...
                    <div class="flex-1">
                        <p class="text-sm font-semibold text-amber-800">🔒 Upskilling Courses Locked</p>
                        <p class="text-sm text-amber-700">Complete and PASS all general courses with a passing score to unlock specialized upskilling courses.</p>
                        <div class="mt-3">
                            <div class="flex justify-between text-xs text-amber-700 mb-1">
                                <span>Progress to Unlock</span>
                                <span><?php echo $passed_general; ?>/<?php echo $total_general; ?> General Courses Passed</span>
                            </div>
                            <div class="w-full bg-amber-200 rounded-full h-2.5">
                                <div class="bg-amber-500 h-2.5 rounded-full" style="width: <?php echo $total_general > 0 ? ($passed_general / $total_general) * 100 : 0; ?>%"></div>
                            </div>
                        </div>
                        <p class="text-xs text-amber-600 mt-2"><i class="fas fa-info-circle mr-1"></i> You need to achieve a <strong>PASSING SCORE</strong> on all general courses (70% or higher and pass ALL quizzes) to unlock these advanced courses.</p>
                    </div>
                </div>
            <?php else: ?>
                <!-- Success banner -->
                <div class="mb-6 bg-green-50 border border-green-200 rounded-xl p-4 flex items-start gap-3">
                    ...
                    <div>
                        <p class="text-sm font-semibold text-green-800">🎉 Congratulations! Upskilling Courses Unlocked! 🎉</p>
                        <p class="text-sm text-green-700">You have successfully passed all general courses. You can now access specialized upskilling courses to advance your career!</p>
                    </div>
                </div>
            <?php endif; ?>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6" id="coursesGrid">
                <?php if (empty($courses)): ?>
                    <div class="col-span-full text-center py-12 bg-white rounded-xl">
                        ...
                        <p>No upskilling courses available.</p>
                    </div>
                <?php else: ?>
                    <?php foreach ($courses as $course): 
                        ...
                        $isInProgress = ($result['course_status'] === 'in_progress' && $result['status'] !== 'approved_retake' && $result['status'] !== 'pending_retake');
                        $isNotStarted = ($result['course_status'] === 'not_started' && $result['status'] !== 'approved_retake' && $result['status'] !== 'pending_retake');
                        $hasApprovedRetake = $result['has_approved_retake'];
                        $hasPendingRetake = $result['has_pending_retake'];
                        $isLocked = !$upskilling_unlocked;
                        ...
                        if (!empty($thumb)) {
                            ...
                            $thumbUrl = 'https://placehold.co/600x300/7c3aed/white?text=' . urlencode($course['title']);
                        }
                    ?>
                    <div class="course-card bg-white rounded-xl shadow-sm overflow-hidden border relative <?php echo $isLocked ? 'opacity-70' : ''; ?>" 
                        ...
                        <?php if ($isLocked): ?>
                            <div class="locked-overlay">
                                <div class="text-center text-white">
                                    ...
                                    <p class="text-sm font-semibold">Complete all general courses first</p>
                        ...
                        <div class="relative h-44">
                            ...
                            <div class="absolute top-3 left-3 z-10">
                                ...
                                <?php elseif ($isInProgress): ?>
                                    <span class="px-3 py-1 rounded-full text-xs font-semibold bg-blue-600 text-white shadow-lg"><i class="fas fa-play text-xs mr-1"></i> IN PROGRESS</span>
                                <?php elseif ($isNotStarted && !$isLocked): ?>
                            ...
                            <?php if ((($isPassed || $isFailed) && $result['average_score'] > 0) && !$hasApprovedRetake && !$hasPendingRetake && !$isLocked): ?>
                            ...
                            <?php if ($isInProgress && !$hasApprovedRetake && !$hasPendingRetake && !$isLocked): ?>
                        ...
                        <div class="p-4 flex-1 flex flex-col">
                            <div class="flex gap-2 mb-2 flex-wrap">
                                <span class="text-xs px-2 py-1 bg-purple-100 text-purple-700 rounded-full"><?php echo htmlspecialchars($course['category'] ?? 'Upskilling'); ?></span>
                                <span class="text-xs px-2 py-1 bg-indigo-100 text-indigo-700 rounded-full"><?php echo htmlspecialchars($course['difficulty'] ?? 'Advanced'); ?></span>
                            </div>
                            ...
                            <?php elseif ($hasPendingRetake): ?>
                                <div class="flex items-center gap-2 text-yellow-700 mb-3 bg-yellow-50 rounded-lg px-3 py-2 text-xs">
                                    <i class="fas fa-clock"></i> Your retake request is pending admin approval.
                                </div>
                                <div class="mt-auto">
                                    <div class="w-full py-2.5 rounded-xl bg-gray-300 text-gray-600 text-sm font-semibold flex items-center justify-center gap-2 cursor-not-allowed">
                                        <i class="fas fa-hourglass-half"></i> Request Pending
                                    </div>
                                </div>
                            <?php elseif ($hasApprovedRetake): ?>
                            ...
                            <?php elseif ($isFailed): ?>
                                <div class="flex items-center gap-2 text-red-600 mb-3 bg-red-50 rounded-lg px-3 py-2 text-xs">
                                    <i class="fas fa-exclamation-triangle"></i> Failed - <?php echo $result['average_score']; ?>% (Need <?php echo $result['passing_threshold']; ?>% to pass all quizzes)
                                </div>
            ...
            <!-- Progress Banner -->
            <?php if ($upskilling_unlocked && $total_courses > 0): ?>
            <div class="mt-8 bg-gradient-to-r from-indigo-50 to-blue-50 rounded-2xl p-5 border border-indigo-100">
                <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4 justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="font-semibold text-gray-800">📈 Your Upskilling Progress</p>
                            <p class="text-sm text-gray-600">Complete upskilling courses to advance your career!</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-2 bg-white rounded-full px-4 py-2 shadow-sm">
                        <i class="fas fa-certificate text-indigo-500"></i>
                        <span class="text-xs font-medium text-gray-700">Certificates Earned: <span class="font-bold text-indigo-600"><?php echo $passed_courses; ?>/<?php echo $total_courses; ?></span></span>
                    </div>
                </div>
            </div>
            <?php endif; ?>
